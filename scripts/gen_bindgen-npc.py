...
def gen_binding_and_process():
    print("bindgen 生成中")
    subprocess.run(
        'bindgen ./crates/npc_bindings/src/exports.h -o ./crates/npc_bindings/src/raw.rs --no-layout-tests --allowlist-item="NPCExports" -- -x c++ -std=c++17',
        shell=True,
    )
    print("bindgen 生成完成, 开始处理 Option")

    target_file = Path("./crates/npc_bindings/src/raw.rs")

    with open(target_file, "r", encoding="utf-8") as bind_file:
        content = bind_file.read()

    option = "::std::option::Option<"
    # raw_cotent begin: pub struct NPCExports {
    # raw_content end: }\n
    raw_start_pos = content.find("pub struct NPCExports {")
