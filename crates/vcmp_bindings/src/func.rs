...
impl VcmpFunctions {
    ...
    */
    ...
    /// 删除车辆
    pub fn delete_vehicle(&self, vehicle_id: i32) -> VcmpResult<()> {
    ...
    /// 设置车辆选项
    pub fn set_vehicle_option(&self, vehicle_id: i32, option: i32, toggle: bool) -> VcmpResult<()> {
        let code = (self.inner.SetVehicleOption)(vehicle_id, option, toggle as u8);
        if code != 0 {
            Err(VcmpError::from(code))
        } else {
            Ok(())
    ...
    /// 获取车辆选项
    pub fn get_vehicle_option(&self, vehicle_id: i32, option: i32) -> bool {
        (self.inner.GetVehicleOption)(vehicle_id, option) != 0
    }

    /// 获取车辆同步源
    pub fn get_vehicle_sync_source(&self, vehicle_id: i32) -> i32 {
        (self.inner.GetVehicleSyncSource)(vehicle_id)
    }

    /// 获取车辆同步类型
    pub fn get_vehicle_sync_type(&self, vehicle_id: i32) -> i32 {
        (self.inner.GetVehicleSyncType)(vehicle_id)
    }

    /// 检查车辆是否对玩家可见
    pub fn is_vehicle_streamed_for_player(&self, vehicle_id: i32, player_id: i32) -> bool {
        (self.inner.IsVehicleStreamedForPlayer)(vehicle_id, player_id) != 0
    }

    /// 设置车辆世界
    pub fn set_vehicle_world(&self, vehicle_id: i32, world: i32) -> VcmpResult<()> {
        let code = (self.inner.SetVehicleWorld)(vehicle_id, world);
        if code != 0 {
            Err(VcmpError::from(code))
        } else {
            Ok(())
    ...
    /// 获取车辆乘客
    pub fn get_vehicle_occupant(&self, vehicle_id: i32, slot_index: i32) -> i32 {
        (self.inner.GetVehicleOccupant)(vehicle_id, slot_index)
    }

    /// 重生车辆
    pub fn respawn_vehicle(&self, vehicle_id: i32) -> VcmpResult<()> {
    ...
    /// 获取车辆免疫标志
    pub fn get_vehicle_immunity_flags(&self, vehicle_id: i32) -> u32 {
        (self.inner.GetVehicleImmunityFlags)(vehicle_id)
    }

    /// 引爆炸弹
    pub fn explode_vehicle(&self, vehicle_id: i32) -> VcmpResult<()> {
    ...
    /// 设置车辆空闲重生计时器
    pub fn set_vehicle_idle_respawn_timer(&self, vehicle_id: i32, millis: u32) -> VcmpResult<()> {
        let code = (self.inner.SetVehicleIdleRespawnTimer)(vehicle_id, millis);
        if code != 0 {
            Err(VcmpError::from(code))
        } else {
            Ok(())
    ...
    /// 获取车辆空闲重生计时器
    pub fn get_vehicle_idle_respawn_timer(&self, vehicle_id: i32) -> u32 {
        (self.inner.GetVehicleIdleRespawnTimer)(vehicle_id)
    }

    /// 设置车辆生命值
    pub fn set_vehicle_health(&self, vehicle_id: i32, health: f32) -> VcmpResult<()> {
    ...
    /// 获取车辆部件状态
    pub fn get_vehicle_part_status(&self, vehicle_id: i32, part_id: i32) -> i32 {
        (self.inner.GetVehiclePartStatus)(vehicle_id, part_id)
    }
    ...
    /// 获取车辆轮胎状态
    pub fn get_vehicle_tyre_status(&self, vehicle_id: i32, tyre_id: i32) -> i32 {
        (self.inner.GetVehicleTyreStatus)(vehicle_id, tyre_id)
    }

    /// 设置车辆损坏数据
    pub fn set_vehicle_damage_data(&self, vehicle_id: i32, damage_data: u32) -> VcmpError {
        let code = (self.inner.SetVehicleDamageData)(vehicle_id, damage_data);
        VcmpError::from(code)
    }

    /// 获取车辆损坏数据
    pub fn get_vehicle_damage_data(&self, vehicle_id: i32) -> u32 {
        (self.inner.GetVehicleDamageData)(vehicle_id)
    }

    /// 设置车辆电台
    pub fn set_vehicle_radio(&self, vehicle_id: i32, radio_id: i32) -> VcmpError {
    ...
    /// 重置所有车辆操控设置
    pub fn reset_all_vehicle_handlings(&self) {
        (self.inner.ResetAllVehicleHandlings)();
    }

    /// 检查操控规则是否存在
    pub fn exists_handling_rule(&self, model_index: i32, rule_index: i32) -> bool {
        (self.inner.ExistsHandlingRule)(model_index, rule_index) != 0
    }

    /// 设置操控规则
    pub fn set_handling_rule(&self, model_index: i32, rule_index: i32, value: f64) -> VcmpError {
        let code = (self.inner.SetHandlingRule)(model_index, rule_index, value);
        VcmpError::from(code)
    }

    /// 获取操控规则
    pub fn get_handling_rule(&self, model_index: i32, rule_index: i32) -> f64 {
        (self.inner.GetHandlingRule)(model_index, rule_index)
    }

    /// 重置操控规则
    pub fn reset_handling_rule(&self, model_index: i32, rule_index: i32) -> VcmpError {
        let code = (self.inner.ResetHandlingRule)(model_index, rule_index);
        VcmpError::from(code)
    }

    /// 重置车辆操控设置
    pub fn reset_handling(&self, model_index: i32) -> VcmpError {
        let code = (self.inner.ResetHandling)(model_index);
        VcmpError::from(code)
    }

    /// 检查实例操控规则是否存在
    pub fn exists_inst_handling_rule(&self, vehicle_id: i32, rule_index: i32) -> bool {
    ...
    /// 获取实例操控规则
    pub fn get_inst_handling_rule(&self, vehicle_id: i32, rule_index: i32) -> f64 {
        (self.inner.GetInstHandlingRule)(vehicle_id, rule_index)
    }

    /// 重置实例操控规则
    pub fn reset_inst_handling_rule(&self, vehicle_id: i32, rule_index: i32) -> VcmpError {
        let code = (self.inner.ResetInstHandlingRule)(vehicle_id, rule_index);
        VcmpError::from(code)
    }

    /// 重置车辆实例操控设置
    pub fn reset_inst_handling(&self, vehicle_id: i32) -> VcmpError {
